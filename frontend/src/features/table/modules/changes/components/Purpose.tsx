...
export const Purpose: FC<Props> = ({ data }) => {
	return (
					...
					<TableBody>
						{data.map(item => (
							<TableRow key={item.id} sx={{ minHeight: 38, cursor: 'default' }}>
								...
								<TableCell width={240}>{JSON.parse(item.original)?.purpose}</TableCell>
								<TableCell width={240}>
									<Typography fontWeight={700}>{JSON.parse(item.changed)?.purpose}</Typography>
								</TableCell>
