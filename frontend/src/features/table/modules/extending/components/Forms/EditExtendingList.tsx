...
import { useGetExtendingQuery } from '../../extendingApiSlice'
import { NoRowsOverlay } from '@/features/table/components/NoRowsOverlay/components/NoRowsOverlay'
import { BoxFallback } from '@/components/Fallback/BoxFallback'
import { LeftArrowIcon } from '@/components/Icons/LeftArrowIcon'
import { EditExtending } from './EditExtendingItem'

type Props = {
	graphite: string
}

export const EditExtendingList: FC<Props> = ({ graphite }) => {
	const [open, setOpen] = useState('')

	const { data, isFetching } = useGetExtendingQuery(graphite, { skip: !graphite })
	...
	return (
		<>
			...
			<List>
				{data?.data.map(e => (
					<Fragment key={e.id}>
						<ListItemButton onClick={openHandler(e.id)} selected={open === e.id} sx={{ borderRadius: 3 }}>
							<ListItemText primary={e.act} />
							<LeftArrowIcon
								fontSize={16}
